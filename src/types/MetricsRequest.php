...
class MetricsRequest
{
    ...
    public function __construct(array $data)
    {
        ...
        if (empty($data['account_key']) || !is_string($data['account_key'])) {
            throw new InvalidArgumentException('The account_key must be provided and must be a string.');
        }

        if (!isset($data['value'])) {
            throw new InvalidArgumentException('The value must be provided and cannot be null.');
        }
        ...
        if (isset($data['time'])) {
            if ($data['time'] instanceof DateTime) {
                ...
                try {
                    ...
                    throw new InvalidArgumentException('The time provided is not a valid DateTime string.');
                }
            } else {
                throw new InvalidArgumentException('The time must be a DateTime object or a valid DateTime string.');
            }
        } else {
            $this->time = null;
        }
